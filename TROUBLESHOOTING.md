...
Gemini API 側の一時的な高負荷で 5xx エラー。ツール側で**自動リトライ (60s → 180s → 300s の 3 回)** を仕込んでいるため、通常は自動回復します。

それでも失敗が続いた場合:

- [ ] 数十分待ってから **Actions タブ → "Daily News Digest" → Run workflow** で手動再実行
- [ ] 翌朝 06:00 / 翌夕 19:00 の cron は通常通り発火するので、放置でも翌回には届く可能性大

⚠️ **GitHub Actions の cron は最大数時間遅延することがある**仕様で、混雑時間帯にぶつかると 503 を引きやすい。これは GitHub と Google 双方の事情で、こちら側で完全に防ぐのは難しい性質のもの。
...
### GitHub Actions の cron は**遅延または skip がある仕様**です。

- GitHub 公式の仕様として、cron の起動が遅れる **だけでなく、混雑時間帯は完全に skip される** ことがあります
- 特に **毎時 00 分付近は世界中の job が集中** して遅延・skip しやすい
- 本テンプレートはこれを避けるため `:13` に設定済み (06:13 JST / 19:13 JST)
- それでも skip された場合は **Actions タブ → Run workflow** で手動再実行 (1 クリックで届く)
- 連日 skip される場合は cron をさらに別の分 (例: `:23`, `:37`) にずらしてみてください

### 📊 実機検証で分かった遅延パターン (率直情報)

無料枠の現実として **2〜6 時間の遅延が常態化** しています:

| 設定時刻 | 実際の配信時刻 | 遅延 | 信頼性 |
|---|---|---|---|
| 朝 06:13 JST | 約 09:00〜09:15 JST | 約 3 時間 | ★★★ ほぼ毎日届く |
| 夕方 19:13 JST | 翌 00:00〜01:30 JST | 約 6 時間 | ★ よく失敗する (Gemini 米国朝ピークと重なる) |

### 夜の配信が失敗しやすい根本原因

夜のクロン (UTC 10:13) が約 6 時間遅延すると、実際の実行時刻は **UTC 16:00 前後**。
これは **米国西海岸の朝 9 時** (PT) で、世界中の Gemini API リクエストが集中する**ピーク時間帯**。
そのため 503 UNAVAILABLE が連発しやすい。

### 改善の選択肢

1. **そのまま無料運用** (失敗時は手動 retry): 多くの人にはこれで十分
2. **クロン時刻をズラす** (`.github/workflows/daily.yml` 編集)
   - 例: 夜のクロンを `13 14 * * *` (= 23:13 JST 設定 → 実際は深夜 5 時頃配信) に
3. **Claude Haiku に切替** (月 300-500 円): Gemini ピーク時間帯の影響を受けない
4. **GitHub Pro に課金** (月 $4): Actions の優先度が上がり遅延が大幅減
